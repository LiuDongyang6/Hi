...
//		}
	...
	public void UpdateKinectToWorldMatrix()
	{
		...
		Vector3 vSensorScale = /*!flipLeftRight ? Vector3.one : new Vector3(-1f, 1f, 1f);*/ new Vector3(1f, -1f, 1f);
...
//					}
	...
	void Update() 
	{
		if(kinectInitialized)
		{
            ...
            //******************************** Save pic ***************************** 
            //System.IO.File.WriteAllBytes(@"D:\codes\test.png", sensorData.colorImageTexture.EncodeToPNG());
            if (HiMusic.MusicDirector.getInstance().colorImageReady)
                if (sensorData.colorImageTexture != null)
                {
                    if (HiMusic.MusicDirector.getInstance().result != null)
                        Debug.Log(HiMusic.MusicDirector.getInstance().result);
                    HiMusic.MusicDirector.getInstance().colorImageReady = false;
                    Thread t = new Thread(() => { HiMusic.MusicDirector.getInstance().receiveColorImage(sensorData.colorImage); });
                    t.Start();
                    if (backClrImg == null)
                    {
                        backClrImg = new byte[sensorData.colorImage.Length];
                    }
                    sensorData.colorImage = backClrImg;
    ...
    protected virtual void CalibrateUser(Int64 userId, int bodyIndex)
    {
		if(!alUserIds.Contains(userId))
		{
			if(CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
			{
                ...
                //DetectGesture(userId, KinectGestures.Gestures.ZoomIn);
                //DetectGesture(userId, KinectGestures.Gestures.ZoomOut);
                //DetectGesture(userId, KinectGestures.Gestures.Push);
                //DetectGesture(userId, KinectGestures.Gestures.Jump);
