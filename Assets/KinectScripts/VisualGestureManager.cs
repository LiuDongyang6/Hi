...
//					   monoScript.enabled)
	...
	void Update() 
	{
		if(isVisualGestureInitialized)
		{
            ...
            //Debug.Log("UserID:  " + primaryUserID);
			if(UpdateVisualGestures(primaryUserID))
			{
				...
				foreach(string gestureName in gestureNames)
				{
					...
					{
						VisualGestureData data = gestureData[gestureName];
                        results.Add(data.gestureName, data.confidence);
                        //Debug.Log(data.gestureName + data.confidence);
					}
				}
                HiMusic.MusicDirector.getInstance().receiveGestureResults(results);
            }
